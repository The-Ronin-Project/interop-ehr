...
class R4ConditionTransformer : ConditionTransformer {
    ...
    override fun transformCondition(condition: Condition, tenant: Tenant): OncologyCondition? {
        ...
        val code = r4Condition.code ?: CodeableConcept()

        val oncologyCondition = OncologyCondition(
            id = r4Condition.id?.localize(tenant),
        ...
        return try {
            validateAndAlert {
                notNull(r4Condition.id) { "no FHIR id" }
                notNull(r4Condition.code) { "No code" }

                merge(oncologyCondition.validate())
            }

            oncologyCondition
        } catch (e: Exception) {
            logger.error(e) { "Unable to transform condition" }
            null
        }
    }
}
