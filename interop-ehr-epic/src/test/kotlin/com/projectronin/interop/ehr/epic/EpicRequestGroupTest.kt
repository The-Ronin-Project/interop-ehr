...
class EpicRequestGroupTest {
    ...
    fun getRequestGroupByFHIRId() {
        ...
        val requestGroup1 = mockk<BundleEntry> {
            every { resource } returns mockk<RequestGroup>(relaxed = true) {
                every { id!!.value } returns "18675309"
            }
        }
        val requestGroup2 = mockk<BundleEntry> {
            every { resource } returns mockk<RequestGroup>(relaxed = true) {
    ...
    fun `chunking works for getByIDs`() {
        ...
        val requestGroup3 = mockk<BundleEntry> {
            every { resource } returns mockk<RequestGroup>(relaxed = true) {
                every { id!!.value } returns "11223344"
            }
        }
        val requestGroup4 = mockk<BundleEntry> {
            every { resource } returns mockk<RequestGroup>(relaxed = true) {
                every { id!!.value } returns "44332211"
            }
        }
        val requestGroup5 = mockk<BundleEntry> {
            every { resource } returns mockk<RequestGroup>(relaxed = true) {
