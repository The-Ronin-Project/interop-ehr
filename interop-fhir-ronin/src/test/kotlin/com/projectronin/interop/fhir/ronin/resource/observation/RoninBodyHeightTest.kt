...
class RoninBodyHeightTest {
    ...
    private val tenantBodyHeightCoding =
        Coding(
            system = CodeSystem.LOINC.uri,
            display = "Body Height".asFHIR(),
            code = Code("bad-body-height"),
        )
    private val tenantBodyHeightConcept =
        CodeableConcept(
            text = "Tenant Body Height".asFHIR(),
            coding = listOf(tenantBodyHeightCoding),
        )
    private val mappedTenantBodyHeightConcept =
        CodeableConcept(
            text = "Body Height".asFHIR(),
            coding = bodyHeightCodingList,
        )
    private val tenantBodyHeightSourceExtension =
        Extension(
    ...
    fun `transform fails for observation with no ID`() {
        ...
        val (transformResponse, _) = roninBodyHeight.transform(observation, tenant)
    ...
    fun `transforms observation with all attributes - maps Observation code`() {
        ...
        val (transformResponse, validation) =
            roninBodyHeight.transform(
                observation,
                tenant,
            )
    ...
    fun `transforms observation with only required attributes - maps Observation code`() {
        ...
        assertEquals(
            CodeableConcept(text = "dataAbsent".asFHIR()),
            transformed.dataAbsentReason,
        )
        ...
        assertEquals(
            listOf<ObservationReferenceRange>(),
            transformed.referenceRange,
        )
