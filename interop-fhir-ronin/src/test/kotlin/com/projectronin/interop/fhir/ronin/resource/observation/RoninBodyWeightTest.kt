...
class RoninBodyWeightTest {
    ...
    fun `validate fails if quantity system is not UCUM`() {
        ...
        assertEquals(
            "Encountered validation error(s):\n" +
                "ERROR USCORE_WTOBS_002: Quantity system must be UCUM @ Observation.valueQuantity.system",
            exception.message
        )
    }
    ...
    fun `transforms observation with all attributes`() {
        val observation = Observation(
            ...
            derivedFrom = listOf(Reference(reference = "Observation/2345".asFHIR())),
            component = listOf(
        ...
        assertEquals(listOf(Reference(reference = "Observation/test-2345".asFHIR())), transformed.derivedFrom)
        assertEquals(
            listOf(
                ObservationComponent(
                    code = CodeableConcept(text = "code2".asFHIR()),
