...
class RoninVitalSignsTest {
    ...
    fun `does not qualify when no category`() {
        ...
        val qualified = RoninVitalSigns.qualifies(observation)
    ...
    @Test
    fun `does not qualify when coding code not for vital signs`() {
        val observation = Observation(
            id = Id("123"),
            status = ObservationStatus.AMENDED.asCode(),
            dataAbsentReason = CodeableConcept(text = "dataAbsent".asFHIR()),
    ...
    @Test
    fun `does not qualify when body height`() {
        val observation = Observation(
            id = Id("123"),
            status = ObservationStatus.AMENDED.asCode(),
            dataAbsentReason = CodeableConcept(text = "dataAbsent".asFHIR()),
    ...
    @Test
    fun `does not qualify when body weight`() {
        val observation = Observation(
            id = Id("123"),
            status = ObservationStatus.AMENDED.asCode(),
            dataAbsentReason = CodeableConcept(text = "dataAbsent".asFHIR()),
    ...
    fun `validate checks ronin identifiers`() {
        ...
        val exception = assertThrows<IllegalArgumentException> {
            RoninVitalSigns.validate(observation, null).alertIfErrors()
        }
    ...
    fun `transforms observation with all attributes`() {
        ...
        assertEquals(Meta(profile = listOf(Canonical(RoninProfile.OBSERVATION_VITAL_SIGNS.value))), transformed.meta)
        ...
        assertEquals(CodeableConcept(text = "vital sign".asFHIR()), transformed.code)
