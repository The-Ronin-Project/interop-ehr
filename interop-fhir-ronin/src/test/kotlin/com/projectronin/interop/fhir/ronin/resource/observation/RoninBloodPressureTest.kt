...
class RoninBloodPressureTest {
    ...
    private val tenantBloodPressureCoding =
        Coding(
            system = CodeSystem.LOINC.uri,
            display = "Bad Blood Pressure".asFHIR(),
            code = Code("bad-blood-pressure"),
        )
    private val tenantBloodPressureConcept =
        CodeableConcept(
            text = "Tenant Blood Pressure".asFHIR(),
            coding = listOf(tenantBloodPressureCoding),
        )
    private val mappedTenantBloodPressureConcept =
        CodeableConcept(
            text = "Blood Pressure".asFHIR(),
            coding = bloodPressureCodingList,
        )
    private val tenantBloodPressureSourceExtension =
        Extension(
    ...
    fun `transform fails for observation with no ID`() {
        ...
        val (transformResponse, _) = roninBloodPressure.transform(observation, tenant)
    ...
    fun `transforms observation with all attributes - maps Observation code`() {
        ...
        val (transformResponse, validation) = roninBloodPressure.transform(observation, tenant)
