...
class RoninBodyTemperatureTest {
    ...
    private val tenantBodyTemperatureCoding =
        Coding(
            system = CodeSystem.LOINC.uri,
            display = "Bad Body Temp".asFHIR(),
            code = Code("bad-body-temp"),
        )
    private val tenantBodyTemperatureConcept =
        CodeableConcept(
            text = "Tenant Body Temp".asFHIR(),
            coding = listOf(tenantBodyTemperatureCoding),
        )
    private val mappedTenantBodyTemperatureConcept =
        CodeableConcept(
            text = "Body Temp".asFHIR(),
            coding = bodyTemperatureCodingList,
        )
    private val tenantBodyTemperatureSourceExtension =
        Extension(
    ...
    private val normRegistryClient =
        mockk<NormalizationRegistryClient> {
            ...
            } returns ValueSetList(bodyTemperatureCodingList, valueSetMetadata)
            every {
                getConceptMapping(
                    tenant,
                    "Observation.code",
    ...
    fun `qualifies for profile`() {
        ...
        val qualified = roninBodyTemperature.qualifies(observation)
    ...
    fun `validate checks ronin identifiers`() {
        val observation =
            Observation(
                ...
                meta =
                    Meta(
                        profile = listOf(Canonical(RoninProfile.OBSERVATION_BODY_TEMPERATURE.value)),
                        source = Uri("source"),
                    ),
                status = ObservationStatus.AMENDED.asCode(),
                dataAbsentReason = CodeableConcept(text = "dataAbsent".asFHIR()),
        ...
        val exception =
            assertThrows<IllegalArgumentException> {
                roninBodyTemperature.validate(observation).alertIfErrors()
            }
    ...
    @Test
    fun `validate fails with subject and type but no data authority extension reference`() {
        val observation =
            Observation(
                id = Id("123"),
                meta =
