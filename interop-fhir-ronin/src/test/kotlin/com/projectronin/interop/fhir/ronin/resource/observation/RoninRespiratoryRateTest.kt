...
class RoninRespiratoryRateTest {
    ...
    private val tenantRespiratoryRateCoding =
        Coding(
            system = CodeSystem.LOINC.uri,
            display = "Respiratory Rate".asFHIR(),
            code = Code("bad-body-height"),
        )
    private val tenantRespiratoryRateConcept =
        CodeableConcept(
            text = "Tenant Respiratory Rate".asFHIR(),
            coding = listOf(tenantRespiratoryRateCoding),
        )
    private val mappedTenantRespiratoryRateConcept =
        CodeableConcept(
            text = "Respiratory Rate".asFHIR(),
            coding = respiratoryRateCodingList,
        )
    private val tenantRespiratoryRateSourceExtension =
        Extension(
    ...
    private val normRegistryClient =
        mockk<NormalizationRegistryClient> {
            ...
            } returns ValueSetList(respiratoryRateCodingList, valueSetMetadata)
            every {
                getConceptMapping(
                    tenant,
                    "Observation.code",
    ...
    fun `qualifies for profile`() {
        ...
        assertTrue(roninRespiratoryRate.qualifies(observation))
    }
    ...
    fun `validate fails if non-repiratory rate code`() {
        ...
        assertEquals(
            "Encountered validation error(s):\n" +
                "ERROR RONIN_OBS_003: Must match this system|code: http://loinc.org|9279-1 @ Observation.code",
            exception.message,
        )
    }
    ...
    fun `transform fails for observation with no ID`() {
        ...
        val (transformResponse, _) = roninRespiratoryRate.transform(observation, tenant)
    ...
    fun `transforms observation with all attributes - maps Observation code`() {
        ...
        val (transformResponse, validation) = roninRespiratoryRate.transform(observation, tenant)
